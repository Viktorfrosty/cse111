# Grading Criteria: Exceeds requirements
# I completed the core requirements and the suggested Exceeding the Requirements (get_formula_name and sum_protons)
...
def main():
    ...
    print(f"The molar mass is: {molar_mass:.5f} grams/mole")
    
    # Print the number of moles.
    print(f"The number of moles in the sample is: {number_of_moles:.5f}")

    # Call the get_formula_name. (Exceeding the Requirements)
    number_of_protons = sum_protons(symbol_quantity_list, periodic_table_dict)

    # Print the formula name. (Exceeding the Requirements)
    print(f"The total protons in the formula are: {number_of_protons}")

    # Call the get_formula_name. (Exceeding the Requirements)
    formula_name = get_formula_name(formula, known_molecules_dict)

    # Print the formula name. (Exceeding the Requirements)
    print(f"the formula {formula} is: {formula_name}")

def make_periodic_table():
    periodic_table_list = {
        # symbol: ["name", atomic mass, atomic number] core and exceeding requirements
        "Ac": [	"Actinium", 227, 89],
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for data in symbol_quantity_list:
        ...
        symbol = data[SYMBOL_INDEX]
        if symbol in periodic_table_dict:
            data = periodic_table_dict[symbol]
            atomic_mass = data[ATOMIC_MASS_INDEX]
        molar_mass_value = atomic_mass * quantity
        total_molar_mass += molar_mass_value
...
def get_formula_name(formula, known_molecules_dict):
    ...
    if formula in known_molecules_dict:
        ...
        formula_name = "Unknow Compound"
    return formula_name

# Index for inner lists in the periodic table
...
def sum_protons(symbol_quantity_list, periodic_table_dict):
    ...
    for data in symbol_quantity_list:
        ...
        if symbol in periodic_table_dict:
            data = periodic_table_dict[symbol]
            atomic_number = data[ATOMIC_NUMBER_INDEX]
        protons_data = atomic_number * quantity
        total_protons += protons_data
